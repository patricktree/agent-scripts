---
name: playwright-testing-cli
description: "Playwright CLI. Automates browser interactions for web testing, form filling, screenshots, and data extraction. Use when the user wants to write Playwright tests and you need to explore things: navigate websites, interact with web pages, fill forms, take screenshots, test web applications, or extract information from web pages."
---
...
> ⚠️ **run-code warning:** Do not use `run-code` for Playwright APIs that return binary data (e.g. `page.screenshot()`, `page.pdf()`). The return value is printed to stdout, which will flood the context with binary. Use the built-in `screenshot` / `pdf` commands instead.
